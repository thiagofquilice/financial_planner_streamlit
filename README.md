...
## Visão geral
O projeto `financial_planner_streamlit` implementa um assistente financeiro de múltiplas etapas construído em Python e Streamlit. Ele orienta o usuário no cadastro dos dados de um negócio (nome, moeda, horizonte de planejamento), estrutura de receitas, custos, despesas operacionais, investimentos e financiamento. Com base nessas entradas, o aplicativo gera projeções, indicadores de viabilidade e relatórios em PDF/Excel.【F:streamlit_app.py†L1-L63】【F:streamlit_app.py†L1034-L1124】【F:streamlit_app.py†L1606-L1883】
...
3. **Etapa 3 – Custos Diretos e Despesas Variáveis**: para cada produto/serviço, inclua itens de custo direto (quantidade, valor unitário e prazo de pagamento) e despesas variáveis associadas (como taxas ou comissões). Esses valores alimentam o cálculo do custo unitário e da margem de contribuição.【F:streamlit_app.py†L1356-L1478】
4. **Etapa 4 – Despesas Operacionais (Opex)**: organize despesas fixas nas categorias Operacionais, Administrativas e de Vendas. Informe descrições e valores mensais para compor o custo fixo utilizado nas análises de ponto de equilíbrio e fluxo de caixa.【F:streamlit_app.py†L1481-L1527】
...
7. **Etapa 7 – Resultados e Análises**: ajuste cenários variando a quantidade vendida e definindo a taxa de desconto para calcular VPL. Visualize DRE anual, fluxo de caixa, indicadores de viabilidade (VPL, TIR, TIRm, payback), ponto de equilíbrio por produto, além de tabelas mensais e anuais em regimes de competência e caixa.【F:streamlit_app.py†L1606-L1870】
