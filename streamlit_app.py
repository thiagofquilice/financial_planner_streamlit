...
def wizard_step3():
    ...
    # Normalize keys loaded from JSON (they may come as strings like "0", "1")
    if isinstance(st.session_state.get("costs"), dict):
        st.session_state.costs = {
            int(k) if str(k).isdigit() else k: v for k, v in st.session_state.costs.items()
        }
    if isinstance(st.session_state.get("variable_expenses"), dict):
        st.session_state.variable_expenses = {
            int(k) if str(k).isdigit() else k: v for k, v in st.session_state.variable_expenses.items()
        }
